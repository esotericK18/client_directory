...
describe Client::CLI do
  ...
  it "prints matching clients for 'search' command" do
    output = capture_stdout { described_class.start(['search', 'john']) }
  ...
  it "prints no clients found message if search returns empty" do
    ...
    output = capture_stdout { described_class.start(['search', 'zzz']) }
  ...
  it "handles extra whitespace in arguments" do
    allow_any_instance_of(Client::Services::SearchClientsService).to receive(:call).with('   john   doe  ').and_return([client1])
